...
export class LandingpageService {
    ...
    async getLandingPageContent(page: string) {
         const titlePage = await this.prisma.titlePage.findFirst({where: {page: page}});
        const content = await this.prisma.selectCard.findMany({where: {page: page}});
        return {
            title: titlePage,
    ...
    async createAndUpdateContentLandingPage(body: contentPageDto, page: string) {
      try {
        await this.prisma.$transaction(async (context) => {
          ...
          await Promise.all([
            body.titlePage?.id
            ? await context.titlePage.update({ where: { id: body.titlePage.id }, data: titleData })
            : await context.titlePage.create({ data: titleData })
          ])

            await Promise.all(body.selectCards.map(async (item) => {
              const data = {
                title: item.title,
              ...
              return item?.id
                ? await context.selectCard.update({ where: { id: item.id }, data })
                : await context.selectCard.create({ data });
            }));
          });
